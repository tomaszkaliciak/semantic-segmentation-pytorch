...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def segm_transform(self, segm):
        ...
        segm = np.asarray(segm, dtype=np.int32).copy()
        copy = segm.copy()
        
        for _voidc  in self.void_classes:
            segm[copy == _voidc] = 5
        for _validc in self.valid_classes:
            segm[copy == _validc] = mapToMainClass(_validc)
...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            scales =  [1, 1.1, 1.2, 1.3, 1.4, 1.5 ,1.6]
            scaleIndex = np.random.randint(0,7)
            newW = int(scales[scaleIndex] *1024)
            newH = int(scales[scaleIndex] * 576)

            img = imresize(img, (newW, newH), interp='bilinear')
            segm = imresize(segm, (newW, newH), interp='nearest')
            new_w = np.random.randint(0, img.size[0]-1023)
            new_h = np.random.randint(0, img.size[1]-575)
...
def mapToMainClass(mapillaryId):
    if mapillaryId in [24]:
        ...
        return 3
    elif mapillaryId in [52, 54, 55, 56, 57, 58, 60, 61]:
        return 4
    else:
        return 5
